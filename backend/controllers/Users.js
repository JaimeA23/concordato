...
const calculardatos= (users) => {
    ...
    var vida=((((((users.raza.constitucion + users.constitucion)*2)+(users.raza.fuerza + users.fuerza)+((users.raza.agilidad + users.agilidad)*0.5))*1.3)+u(sers.raza.espiritu + users.espiritu)+((users.raza.poder + users.poder)*0.5))+(users.raza.bon_salud)).toFixed()*1;
